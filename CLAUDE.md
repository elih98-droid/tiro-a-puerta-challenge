...
- **v5.5 (mayo 2026):** sesión 18 de mayo — auditoría pre-lanzamiento y preparación para usuarios reales. **Leaderboard:** "pick de hoy" por usuario (solo post-deadline, `effective_deadline <= now`), título "Ranking — Top 50" con subtítulo. **Dashboard:** `WelcomeCard` pre-Mundial ("Todos dicen que conocen a la bocha. Demuéstralo.") — se muestra antes del 11 jun; `RestCard` original preservada para días sin partidos durante el Mundial. **World Cup seeds:** 48 equipos (fix códigos duplicados AUS/AUT, IRA/IRN/IRQ, CUW null) y 72 partidos de fase de grupos (17 match days, jun 11-27). **Countdown al Mundial** en dashboard (auto-oculta post-kickoff). **Rivals en vivo:** stats de jugadores (tiros, goles, minuto, badge de estado) con polling 60s. **Auditoría pre-lanzamiento completa** — 7 fixes: (1) `/update-password` removido de `AUTH_ONLY_ROUTES` (rompía reset de contraseña), (2) errores OAuth visibles en login (`?error=` search params), (3) reenvío de email de confirmación (`resendConfirmationEmail` action + botón en login), (4) password login >= 8 (alineado con signup), (5) `noValidate` en form de login, (6) tiebreaker estable `user_id ASC` en ranking, (7) pick page empty state estilizado ("Sabemos que estás ansioso, ya casi arranca"). **Tiebreaker final:** empate en Top 5 = premios compartidos, sin sorteo (`game-rules.md §5.3`). **App lista para registros reales.**

- **v5.4 (mayo 2026):** sesión 16 de mayo — auditoría completa de seguridad, lógica, performance y reliability. **6 fixes + 1 feature + 1 optimización:** (1) `submitPick` ya no acepta `matchId`/`effectiveDeadline` del cliente — solo `playerId` + `matchDayId`, servidor valida todo contra DB. (2) `closeMatchDay` + `getAliveApprovedUserIds()` helper — pre-picks de eliminados no bloquean cierre de día; recalculado dentro del loop. (3) Open redirect en `/auth/callback` — `next` param validado como path relativo. (4) Dashboard rank filtra `is_approved` — consistente con leaderboard. (5) `check-lineups` no marca `lineups_notified` si emails fallaron — reintenta en próxima corrida. (6) `evaluate-picks` verifica que `player_match_stats` exista antes de evaluar — si sync falló, skipea partido en vez de eliminar injustamente. (7) **Partidos suspendidos (contingencia tormentas):** `sync-live-matches` incluye `suspended` en la query; force-finish consulta API antes de forzar (si INT/SUSP → marca suspended, no finished); dashboard muestra badge "SUSPENDIDO" amber con stats acumulados; polling continúa hasta que reanude. (8) **Optimización API:** skip `getFixturePlayers` durante suspensiones (stats no cambian), ahorra 60 calls/hora → ~77h de margen. **Lint limpio:** 2 errores + 6 warnings resueltos. **Auditoría completa documentada** con hallazgos pendientes (ver ROADMAP §15).

- **v5.3 (mayo 2026):** sesión 15 de mayo (cierre) — emails de contacto y limpieza. **Emails `privacidad@tiroapuerta.mx` y `contacto@tiroapuerta.mx`** configurados via Cloudflare Email Routing → reenvío a `elih98@gmail.com`. Registros MX + TXT agregados automáticamente. Pendiente de T&C cerrado. **Alertas de DB** descartadas del roadmap — overengineering para 300-400 usuarios (Supabase Pro tiene monitoreo básico en dashboard). **Revive masivo de beta testers** para pruebas PL del domingo: reset de `user_status` + `match_days.is_processed = TRUE` en días pasados para evitar re-eliminación por `evaluate-picks`.

- **v5.2 (mayo 2026):** sesión 15 de mayo (continuación) — sistema anti-trampa. **Rate limiting** (`lib/rate-limit.ts`): sliding-window counter en memoria por IP, sin librería externa. Umbrales: signIn 5/15min, signUp 3/60min, resetPassword 3/60min, submitPick 20/5min, removePick 20/5min. Fail-open en cold start (aceptable — aprobación manual es la barrera real). Mensaje de error en español con tiempo de espera. **Device fingerprinting:** `lib/utils/hash.ts` (SHA-256 truncado 16 hex chars para UA). `pick_history.ip_address` e `pick_history.user_agent_hash` (columnas existentes, antes NULL) ahora poblados desde Server Actions vía admin client. `signIn` guarda `last_login_ip` y `last_login_ua_hash` en `user_metadata` de Supabase Auth. **Alertas automáticas** (`lib/admin/alerts.ts`): detecta (1) misma IP con ≥2 cuentas/día → posible multi-cuenta, (2) ≥5 cambios de pick en 10 min → posible bot. Email a los 3 admins con template Dirección 3 (`suspicious-activity.ts`) banner rojo, detalles, nota de falso positivo, CTA admin panel. Dedup en memoria 24h para no spammear. CAPTCHA descartado (aprobación manual ya filtra bots). Sin migraciones de DB (todo sobre columnas existentes + user_metadata). 8vo email del sistema.
