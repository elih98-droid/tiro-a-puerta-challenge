...
- **v5.4 (mayo 2026):** sesión 16 de mayo — auditoría de seguridad y reliability. **Fix 1 — Validación server-side incompleta en `submitPick`:** el servidor aceptaba `matchId`, `matchDayId` y `effectiveDeadline` del cliente sin verificar contra DB. Un usuario técnico podía forjar combinaciones inválidas (jugador de otro partido, deadline falso). Fix: `submitPick` ahora solo recibe `playerId` y `matchDayId`; el servidor busca el jugador (confirma activo), encuentra el partido (confirma que el equipo juega ese día, valida exactamente 1 resultado), y usa el `pick_deadline` real de DB. **Fix 2 — Pre-picks de eliminados bloquean cierre de día:** `closeMatchDay` verificaba picks pendientes (`is_locked = true, result = null`) sin filtrar por usuarios vivos/aprobados. Pre-picks de usuarios eliminados impedían `is_processed = TRUE` indefinidamente. Fix: helper `getAliveApprovedUserIds()` extraído y reutilizado en Phase A y Phase B; se re-llama después de Phase A para reflejar eliminaciones recientes; `closeMatchDay` recibe el array y solo cuenta picks de usuarios relevantes.
