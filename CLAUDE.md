...
- **v3.0 (abril 2026):** responsividad desktop. Game layout (`app/(game)/layout.tsx`) ahora centra todo el contenido a `max-width: 480px` con `margin: 0 auto`. Brand bar tiene fondo/borde full-width pero contenido centrado. Confirm panel sticky de pick corregido de `maxWidth: 600` → `480`. Nav ya tenía `maxWidth: 480`. Auth pages (login/signup) ya tenían `maxWidth: 460` — sin cambios. Resultado: en desktop el contenido aparece centrado como app mobile, fondo oscuro de borde a borde. Eliminados todos los estilos Tailwind/blancos. Panel oscuro `rgba(0,0,0,0.3)` con borde sutil, integrado visualmente dentro de `PickConfirmedCard`. Score en Bebas Neue 20px. Stats (tiros/goles/minutos) en Bebas Neue 28px con glow de color (verde tiros, dorado goles). Badges de estado (EN VIVO / FINALIZADO / SUSPENDIDO) en JetBrains Mono con bordes de color. Survival indicator en JetBrains Mono uppercase. Timestamp de sync dim 9px. Mismo lenguaje visual que login: fondo oscuro `#0B0D18`, overlay grid, halo dorado, esquinas HUD. `TPMark` 72px (vs 92px en login para dar espacio a más campos). 3 campos: email (EmailIcon), username (UserIcon + hint de formato), password (toggle show/hide con EyeIcon/EyeOffIcon). 2 `DarkCheckbox` custom (controlled, checkmark dorado): `over_18_confirmed` (requerido) y `marketing_emails_opt_in` (opcional con hint). CTA signature (gradiente azul + borde dorado) "Crear cuenta". `OAuthButtons` FUERA del `<form>` (evita nested form). Link "¿Ya tienes cuenta?" en dorado. Validación en cliente con estado blurred. Build ✅ limpio.
