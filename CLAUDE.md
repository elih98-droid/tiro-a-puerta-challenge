...
- **v5.4 (mayo 2026):** sesión 16 de mayo — auditoría completa de seguridad, lógica, performance y reliability. **6 fixes aplicados:** (1) `submitPick` ya no acepta `matchId`/`effectiveDeadline` del cliente — solo `playerId` + `matchDayId`, servidor valida todo contra DB. (2) `closeMatchDay` + `getAliveApprovedUserIds()` helper — pre-picks de eliminados no bloquean cierre de día; recalculado dentro del loop. (3) Open redirect en `/auth/callback` — `next` param validado como path relativo (no `//` ni absoluto). (4) Dashboard rank filtra `is_approved` — consistente con leaderboard. (5) `check-lineups` no marca `lineups_notified` si emails fallaron — reintenta en próxima corrida. (6) `evaluate-picks` verifica que `player_match_stats` exista antes de evaluar — si sync falló, skipea partido en vez de eliminar injustamente. **Lint limpio:** 2 errores + 6 warnings resueltos (polling setTimeout, unused vars, DarkCheckbox id). **Auditoría completa documentada** con hallazgos pendientes para resolver antes del Mundial (ver ROADMAP §15).
